...
def load_and_clean_data(file_path):
    ...
    df = df[(df['price'] > 100) & (df['price'] < 5000)]  # Assuming ETH price is between $100 and $5000
    
    print(f"Data points after removing extreme prices: {len(df)}")
    
    # Calculate price changes
    df['price_change'] = df['price'].pct_change()
    
    # Remove extreme price changes
    df = df[df['price_change'].abs() < 0.1]  # Remove changes greater than 10%
    
    print(f"Final data points: {len(df)}")
    
    return df

def simulate_trading(df, initial_eth, trigger_percentage):
    position = "ETH"
    ...
    for _, row in df.iterrows():
        ...
        if position == "ETH" and price_change >= trigger_percentage:
            usdc_amount = eth_amount * current_price
            eth_amount = 0
            position = "USDC"
            trades.append((current_time, "Sell ETH", current_price, usdc_amount))
            entry_price = current_price
            print(f"Sold ETH at ${current_price:.2f}, USDC balance: ${usdc_amount:.2f}")

        elif position == "USDC" and price_change <= -trigger_percentage:
            eth_amount = usdc_amount / current_price
            usdc_amount = 0
            position = "ETH"
            trades.append((current_time, "Buy ETH", current_price, eth_amount))
            entry_price = current_price
            print(f"Bought ETH at ${current_price:.2f}, ETH balance: {eth_amount:.6f}")
...
def main():
    ...
    initial_eth = 1  # Starting with 1 ETH
    initial_value = initial_eth * df['price'].iloc[0]
    trigger_percentages = [0.03, 0.05, 0.07, 0.10]
    
    results = []
    
    for trigger in trigger_percentages:
        print(f"\nSimulating with {trigger*100}% trigger:")
        trades, portfolio_values, final_value = simulate_trading(df, initial_eth, trigger)
